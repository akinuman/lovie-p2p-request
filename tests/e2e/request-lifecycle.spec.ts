...
test.describe("Request Lifecycle — Pay", () => {
  test("recipient pays a request with confirmation dialog", async ({ signInAs, page }) => {
    ...
    await page.getByRole("textbox", { name: "For" }).fill("E2E pay test unique note");
    ...
    const requestCard = page.getByTestId("incoming-request-card").filter({
      hasText: "E2E pay test unique note",
    }).first();
    await expect(requestCard).toBeVisible();

    // Click Pay — opens confirmation dialog
    await requestCard.getByRole("button", { name: /pay request/i }).first().click();
    ...
    await signInAs(demoUsers.sender);
    const paidCard = page.getByTestId("outgoing-request-card").filter({
      hasText: "E2E pay test unique note",
    }).first();
...
test.describe("Request Lifecycle — Decline", () => {
  test("recipient declines a request", async ({ signInAs, page }) => {
    ...
    await page.getByRole("textbox", { name: "To" }).fill(demoUsers.recipient);
    await page.getByRole("textbox", { name: "For" }).fill("E2E decline test unique note");
    await page.getByRole("button", { name: "Create request" }).click();
    await expect(page).toHaveURL(/\/dashboard\/outgoing/);
    await page.getByRole("dialog").getByRole("button", { name: "Close" }).click();
    ...
    await requestCard.getByRole("button", { name: /decline/i }).first().click();
    ...
    await signInAs(demoUsers.sender);
    const declinedCard = page.getByTestId("outgoing-request-card").filter({
      hasText: "E2E decline test unique note",
    }).first();
...
test.describe("Request Lifecycle — Cancel", () => {
  test("sender cancels a pending request", async ({ signInAs, page }) => {
    ...
    await page.getByRole("textbox", { name: "To" }).fill(demoUsers.recipient);
    await page.getByRole("textbox", { name: "For" }).fill("E2E cancel test unique note");
    await page.getByRole("button", { name: "Create request" }).click();
    await expect(page).toHaveURL(/\/dashboard\/outgoing/);
    await page.getByRole("dialog").getByRole("button", { name: "Close" }).click();

    // Cancel the request
    const requestCard = page.getByTestId("outgoing-request-card").filter({
    ...
    const cancelledCard = page.getByTestId("outgoing-request-card").filter({
      hasText: "E2E cancel test unique note",
    }).first();
...
test.describe("Request Lifecycle — Expiration", () => {
  test("expired request shows Expired status and no Pay button", async ({ signInAs, page }) => {
    await signInAs(demoUsers.recipient);
    await page.goto("/dashboard/incoming");
    ...
    const expiredCard = page.getByTestId("incoming-request-card").filter({
      hasText: "Expired",
    }).first();
    ...
    if (count > 0) {
      ...
      await expect(expiredCard.getByRole("button", { name: /pay/i })).not.toBeVisible();
    }
  });
});

test.describe("Request Detail View", () => {
  test("shows request details with amount, note, status, and timestamps", async ({ signInAs, page }) => {
    await signInAs(demoUsers.sender);
    ...
    await expect(page.getByText(/pending|paid|declined|cancelled|expired/i).first()).toBeVisible();
  });

  test("incoming request detail shows Pay and Decline for pending", async ({ signInAs, page }) => {
    await signInAs(demoUsers.recipient);
    await page.goto("/dashboard/incoming");
